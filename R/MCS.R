...
MCSprocedure<-function(Loss,alpha=0.15,B=5000,cl=NULL,ram.allocation=T,statistic="Tmax",k=NULL,min.k=3,verbose=T){
  ...
  repeat{
    ...
    if(cl_number>1){
      d_ij_avg_resampled=parLapply(cl[1:cl_number],indexes_b,function(x,d,N){ colSums((d[x,]))/N},d=d,N=N)
    }else{
      d_ij_avg_resampled=lapply(indexes_b,function(x,d,N){ colSums((d[x,]))/N},d=d,N=N)
    }
