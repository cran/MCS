...
boot.block<-function(x,v,n,k){
  tau_b=sapply(1:(v-1),function(y) ceiling(runif(1,1,n)))
  tau_a=matrix(,k,length(tau_b))
  tau_a[1,]=tau_b
  for(i in 2:nrow(tau_a)) tau_a[i,]= tau_a[i-1,]+1
  tau_a[which(tau_a>n)]=tau_a[which(tau_a>n)]-n
  return(c(tau_a,as.integer(runif(n-nrow(tau_a)*ncol(tau_a),1,n))))
}
...
MCSprocedure<-function(Loss,alpha=0.15,B=5000,cl=NULL,ram.allocation=T,statistic="Tmax",k=NULL,min.k=3,verbose=T){
  ...
  if(any(is.na(Loss))) stop("NAs in Loss not allowed")
  if(any(abs(Loss)==Inf)) stop("Inf in Loss not allowed")
  ...
  repeat{
    ...
    temp1=1:M
    temp2=do.call(c,lapply(1:M,function(x) paste(temp1[x],temp1[-x],sep=".")))
    temp3=matrix(as.numeric(unlist(strsplit(temp2,split=".",fixed=T))),ncol=2,byrow=T)
    
    index=col.names.d[temp3[,1]<temp3[,2]]
    ...
    Tb_R=sapply(1:B,function(i) max(abs(d_ij_avg_resampled[i,]-d_ij_mean)/(d_var_ij^0.5)))
      ...
      #manca per semiquadratic
      #calcolo i singoli p-values
